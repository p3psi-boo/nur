# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  bb-browser = {
    pname = "bb-browser";
    version = "bb-browser-v0.11.2";
    src = fetchFromGitHub {
      owner = "epiral";
      repo = "bb-browser";
      rev = "bb-browser-v0.11.2";
      fetchSubmodules = false;
      sha256 = "sha256-DoAd8oN4Il5xGHkcDKgmLczUYj0D0PvxhbSmnDYb+wI=";
    };
  };
  bbdown = {
    pname = "bbdown";
    version = "1b2fbd4372d0b9840d28072f7914ae9887508e5d";
    src = fetchgit {
      url = "https://github.com/nilaoda/BBDown.git";
      rev = "1b2fbd4372d0b9840d28072f7914ae9887508e5d";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-GrBD4PptkryCfpyvnRTPoNfvnT2hAUhZQeS+0SQwJzo=";
    };
    date = "2026-05-14";
  };
  bcut-asr = {
    pname = "bcut-asr";
    version = "07fc48213a415490da1ae40cd8988a087cb02ec6";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/bcut-asr-go.git";
      rev = "07fc48213a415490da1ae40cd8988a087cb02ec6";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-uGhqogSabFpuWQ1h88oXrCOPuJPyfuR9nx8Ug5WbUl0=";
    };
    date = "2026-01-14";
  };
  bichon = {
    pname = "bichon";
    version = "1.6.1";
    src = fetchFromGitHub {
      owner = "rustmailer";
      repo = "bichon";
      rev = "1.6.1";
      fetchSubmodules = false;
      sha256 = "sha256-IWtJ/FUAysAasfNowQZfewhx2brzXGSW4EZtQbijlfE=";
    };
  };
  binsider-bin = {
    pname = "binsider-bin";
    version = "0.3.2";
    src = fetchurl {
      url = "https://github.com/orhun/binsider/releases/download/v0.3.2/binsider-0.3.2-x86_64-unknown-linux-musl.tar.gz";
      sha256 = "sha256-/TScWAgk8rUv981qMCqqd0o/5L7t2TeFFb3+zsDYd6M=";
    };
  };
  binsider-bin-aarch64-linux = {
    pname = "binsider-bin-aarch64-linux";
    version = "0.3.2";
    src = fetchurl {
      url = "https://github.com/orhun/binsider/releases/download/v0.3.2/binsider-0.3.2-aarch64-unknown-linux-musl.tar.gz";
      sha256 = "sha256-pedF8eMfFmfsFPYjm6w8McxQQ5YmJLEtrkGC0xtSROI=";
    };
  };
  binsider-bin-darwin-arm64 = {
    pname = "binsider-bin-darwin-arm64";
    version = "0.3.2";
    src = fetchurl {
      url = "https://github.com/orhun/binsider/releases/download/v0.3.2/binsider-0.3.2-aarch64-apple-darwin.tar.gz";
      sha256 = "sha256-CNYBoXtsEpvWzXGqp2+loZT88hgVzLocm//4tklnT78=";
    };
  };
  binsider-bin-darwin-x64 = {
    pname = "binsider-bin-darwin-x64";
    version = "0.3.2";
    src = fetchurl {
      url = "https://github.com/orhun/binsider/releases/download/v0.3.2/binsider-0.3.2-x86_64-apple-darwin.tar.gz";
      sha256 = "sha256-HDz+JDAf3WeezxZTeLsTNb9kAdAHfPOxu6/lkchG6Bo=";
    };
  };
  bird-cli = {
    pname = "bird-cli";
    version = "c815bfbe3cefd0104adf411cd2148995c31f2ba0";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/bird-fork.git";
      rev = "c815bfbe3cefd0104adf411cd2148995c31f2ba0";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-47N0//wE3G9RGrcSa0JJTnu9mbw/MK58PNnuqYyitvw=";
    };
    date = "2026-03-09";
  };
  browseros = {
    pname = "browseros";
    version = "0.46.0";
    src = fetchurl {
      url = "https://github.com/browseros-ai/BrowserOS/releases/download/v0.46.0/BrowserOS_v0.46.0_x64.AppImage";
      sha256 = "sha256-IV3Agg5i4TKDMQy+BjNbW6sqPsLfPcSOQkOADHhSmTw=";
    };
  };
  classyshark = {
    pname = "classyshark";
    version = "8.2";
    src = fetchFromGitHub {
      owner = "google";
      repo = "android-classyshark";
      rev = "8.2";
      fetchSubmodules = false;
      sha256 = "sha256-9gENYNWfbPK2jpxiHOf8ETJwXqjFMgplaHNkO3ohhDQ=";
    };
  };
  clihub = {
    pname = "clihub";
    version = "v0.0.8";
    src = fetchFromGitHub {
      owner = "thellimist";
      repo = "clihub";
      rev = "v0.0.8";
      fetchSubmodules = false;
      sha256 = "sha256-Y/O2XmhMYUCV269g+h+DsQT80yVR0ot9RIS+mBJP7BA=";
    };
  };
  clipboard-sync = {
    pname = "clipboard-sync";
    version = "0.2.0";
    src = fetchFromGitHub {
      owner = "dnut";
      repo = "clipboard-sync";
      rev = "0.2.0";
      fetchSubmodules = false;
      sha256 = "sha256-gme5pwQrwQbk8MroF/mGYqlY6hcjM5cHKHL7Y3nlW9k=";
    };
  };
  cliproxyapi = {
    pname = "cliproxyapi";
    version = "v7.2.47";
    src = fetchFromGitHub {
      owner = "router-for-me";
      repo = "CLIProxyAPI";
      rev = "v7.2.47";
      fetchSubmodules = false;
      sha256 = "sha256-bQYzaLi5DIP3gCnZ5LyqO1iyGHiOWsiWen+dfQBEWOU=";
    };
  };
  cocoindex-code = {
    pname = "cocoindex-code";
    version = "0.2.37";
    src = fetchurl {
      url = "https://github.com/cocoindex-io/cocoindex-code/archive/refs/tags/v0.2.37.tar.gz";
      sha256 = "sha256-+Lt0w/ATc2TYA1Qe/TAWr/PnstHXvUizlIxEY1JmYfc=";
    };
  };
  coe = {
    pname = "coe";
    version = "c326261675a2455e6dd8c22576681e91644600ef";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/coe.git";
      rev = "c326261675a2455e6dd8c22576681e91644600ef";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-0VL8NsKi1zl8fWJHn9khUwb0Jx0NIEvAdqe7slSfPyw=";
    };
    date = "2026-04-23";
  };
  commandcode2api = {
    pname = "commandcode2api";
    version = "30a6adc138b118ffda394ba162cf26d85fa1f13f";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/commandcode2api.git";
      rev = "30a6adc138b118ffda394ba162cf26d85fa1f13f";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-g12WHKkkoZO8OSyypm1EGEIKJxhpvJv7yqvV5NUTHdw=";
    };
    date = "2026-06-03";
  };
  cookiecloud-rs = {
    pname = "cookiecloud-rs";
    version = "f8a023f42951c8ba97261d3b16e3a4ff53c91f4a";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/CookieCloud.git";
      rev = "f8a023f42951c8ba97261d3b16e3a4ff53c91f4a";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-ak78lDOAaDWCk9dNpQ9nNObwXrhj71umhzu01hagt2I=";
    };
    date = "2026-04-14";
  };
  copilot-api-plus = {
    pname = "copilot-api-plus";
    version = "b7b610f59c48897d223d320fd8e44740ade8f131";
    src = fetchgit {
      url = "https://github.com/imbuxiangnan-cyber/copilot-api-plus.git";
      rev = "b7b610f59c48897d223d320fd8e44740ade8f131";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-QRRrQ455FAtaK3aM7vyss5aeKpe/iQ0rTwwhBKioI6k=";
    };
    date = "2026-06-24";
  };
  dagu = {
    pname = "dagu";
    version = "v2.8.3";
    src = fetchFromGitHub {
      owner = "dagu-org";
      repo = "dagu";
      rev = "v2.8.3";
      fetchSubmodules = false;
      sha256 = "sha256-aQZ9QMYU0wWQROFU8anWXRJYj6VOdnKY38/Wu2QbnNA=";
    };
  };
  dalvikus = {
    pname = "dalvikus";
    version = "1.0.13";
    src = fetchurl {
      url = "https://github.com/loerting/dalvikus/archive/refs/tags/v1.0.13.tar.gz";
      sha256 = "sha256-PMSalWlMDIqMWgPZ6Ha7L6TA2VOX6svCAOYMSpzIqHY=";
    };
  };
  dbhub = {
    pname = "dbhub";
    version = "0.21.2";
    src = fetchurl {
      url = "https://registry.npmjs.org/@bytebase/dbhub/-/dbhub-0.21.2.tgz";
      sha256 = "sha256-eOwzRCZAYypW/GY1zXIqusads2xEv50s01xhfroZPTo=";
    };
  };
  deepseek-ocr-rs = {
    pname = "deepseek-ocr-rs";
    version = "02b933df24f5658d10b37dd48c9c354d95c530c3";
    src = fetchgit {
      url = "https://github.com/TimmyOVO/deepseek-ocr.rs.git";
      rev = "02b933df24f5658d10b37dd48c9c354d95c530c3";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-A15gHXKIi/EieK1JpYLedUd63jCfh19S+1e2TS2cdjQ=";
    };
    date = "2026-02-12";
  };
  ecloud-computer-auto-boot = {
    pname = "ecloud-computer-auto-boot";
    version = "1.0.0";
    src = fetchFromGitHub {
      owner = "Samler-Lee";
      repo = "ecloud_computer_auto_boot";
      rev = "1.0.0";
      fetchSubmodules = false;
      sha256 = "sha256-9H11Jli1Ls8FNtvfgt5X+1yEw4zcrqcmBWPdyq8TIf8=";
    };
  };
  einat = {
    pname = "einat";
    version = "30e6af3740e279efb254568c0ec1ad54c0cccd35";
    src = fetchgit {
      url = "https://github.com/EHfive/einat-ebpf.git";
      rev = "30e6af3740e279efb254568c0ec1ad54c0cccd35";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-QPvyJGd35V5D8Q9mZUPNt8kfpG75qNHxr30oQkfDcV0=";
    };
    date = "2026-06-27";
  };
  fdb = {
    pname = "fdb";
    version = "v1.9.0";
    src = fetchFromGitHub {
      owner = "andrzejchm";
      repo = "fdb";
      rev = "v1.9.0";
      fetchSubmodules = false;
      sha256 = "sha256-2DRMbYHf8dVTiEmggcPRh1oaDhNlxqB+bH9QhKpNi0Q=";
    };
  };
  firm = {
    pname = "firm";
    version = "4144d4a08bdf285c09ed2b9781e5ec448004e506";
    src = fetchgit {
      url = "https://github.com/42futures/firm.git";
      rev = "4144d4a08bdf285c09ed2b9781e5ec448004e506";
      fetchSubmodules = true;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-PgFYlw9XNxcPlRXLIBwfVV+geu3NZ9y2+puN6FczIfg=";
    };
    date = "2026-03-11";
  };
  fish-ai = {
    pname = "fish-ai";
    version = "v2.13.1";
    src = fetchFromGitHub {
      owner = "Realiserad";
      repo = "fish-ai";
      rev = "v2.13.1";
      fetchSubmodules = false;
      sha256 = "sha256-ZSFoE9/UesA6GVSYyRKAfj7+uw1gTZ/E08zAHPizAAQ=";
    };
  };
  focaltech-fingerprint = {
    pname = "focaltech-fingerprint";
    version = "d4fbbf901aff44b92d4fa212d9b99e43cda00563";
    src = fetchgit {
      url = "https://github.com/vobademi/FTEXX00-Ubuntu.git";
      rev = "d4fbbf901aff44b92d4fa212d9b99e43cda00563";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-0mnXEQjSWUrz8uSwaeWH6uLLZtGnSUx5OPBxjPNxFTs=";
    };
    date = "2025-10-21";
  };
  fusion = {
    pname = "fusion";
    version = "ecc44eeff94dd4836eed6490a497989194effa2b";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/fusion.git";
      rev = "ecc44eeff94dd4836eed6490a497989194effa2b";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-O9NMromcyvcF97Kb084vrHPjqRgyRqq14Ay8pdsj9rQ=";
    };
    date = "2026-04-12";
  };
  gemma-cpp = {
    pname = "gemma-cpp";
    version = "e93b7c56f0d6f3714250482b6a91a027350d0f78";
    src = fetchgit {
      url = "https://github.com/test1111111111111112/llama-cpp-turboquant-gemma4.git";
      rev = "e93b7c56f0d6f3714250482b6a91a027350d0f78";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-ykzroJr8zXnLe6Wq279/+ltTxbeSEvcbTufz2EyqYqQ=";
    };
    date = "2026-04-04";
  };
  grdpwasm = {
    pname = "grdpwasm";
    version = "ab8580656c1c9055caf774e72b46a6d4c262627b";
    src = fetchgit {
      url = "https://github.com/nakagami/grdpwasm.git";
      rev = "ab8580656c1c9055caf774e72b46a6d4c262627b";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-SJbYMg8Gd//PT1y5SVgbmTjuMUGXqr/CJEIYKtpa+W4=";
    };
    date = "2026-06-13";
  };
  grok2api = {
    pname = "grok2api";
    version = "14b0937600847d71c4235dde34573d7835b5007e";
    src = fetchgit {
      url = "https://github.com/jiujiu532/grok2api.git";
      rev = "14b0937600847d71c4235dde34573d7835b5007e";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-g5MqhTTbysvFXTYkGP/09EyDi5s8vLcz887lGb73Dpw=";
    };
    date = "2026-06-23";
  };
  harlequin-mysql = {
    pname = "harlequin-mysql";
    version = "v1.3.0";
    src = fetchFromGitHub {
      owner = "tconbeer";
      repo = "harlequin-mysql";
      rev = "v1.3.0";
      fetchSubmodules = false;
      sha256 = "sha256-yWPysa0+GYq4b6WpjdyONTev5CbORsAdQ2JjZuumZv4=";
    };
  };
  hev-socks5-server = {
    pname = "hev-socks5-server";
    version = "2.12.0";
    src = fetchgit {
      url = "https://github.com/heiher/hev-socks5-server.git";
      rev = "2.12.0";
      fetchSubmodules = true;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-8U6NXHiL3ZFb+HuBPm4RKLyKn8YNOQg7DJivsOv0Y5w=";
    };
  };
  ida-mcp = {
    pname = "ida-mcp";
    version = "9.3.24";
    src = fetchurl {
      url = "https://github.com/blacktop/ida-mcp-rs/releases/download/v9.3.24/ida-mcp_9.3.24_Linux_x86_64.tar.gz";
      sha256 = "sha256-Ls9ZaPWTFA82waOtndF4lN/ZDC9P1PfEGJxMkhXqNiQ=";
    };
  };
  ida-pro-mcp = {
    pname = "ida-pro-mcp";
    version = "f4754e45e7d76bfcebb5dcbe700577fa52ff71cd";
    src = fetchgit {
      url = "https://github.com/QiuChenly/ida-pro-mcp-enhancement.git";
      rev = "f4754e45e7d76bfcebb5dcbe700577fa52ff71cd";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-c4kb2qwNrWI/r7teRfBG02ONdA3nxbA0Izl1BiWxABg=";
    };
    date = "2026-06-15";
  };
  il2cppdumper = {
    pname = "il2cppdumper";
    version = "v6.7.46";
    src = fetchFromGitHub {
      owner = "Perfare";
      repo = "Il2CppDumper";
      rev = "v6.7.46";
      fetchSubmodules = false;
      sha256 = "sha256-pMxxwBpuZ2EuP7O99XTJcnS3Dq8MdxvUGQHJ0U3bnTY=";
    };
  };
  inet-speed-cli = {
    pname = "inet-speed-cli";
    version = "v1.1.2";
    src = fetchFromGitHub {
      owner = "tsosunchia";
      repo = "iNetSpeed-CLI";
      rev = "v1.1.2";
      fetchSubmodules = false;
      sha256 = "sha256-xAXYhvN7rdXvA2EWsNXRfXlIrumyPluAcmVbMY/pZxE=";
    };
  };
  jarm = {
    pname = "jarm";
    version = "2c0cf5ce8418c7a1d03edb219acea3c18e068289";
    src = fetchgit {
      url = "https://github.com/salesforce/jarm.git";
      rev = "2c0cf5ce8418c7a1d03edb219acea3c18e068289";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-PPtGS2pYXZga0eYtrKeg4Q/XpIhhTuuILZF+Bw8203c=";
    };
    date = "2026-06-02";
  };
  jd-gui = {
    pname = "jd-gui";
    version = "v1.6.6";
    src = fetchFromGitHub {
      owner = "java-decompiler";
      repo = "jd-gui";
      rev = "v1.6.6";
      fetchSubmodules = false;
      sha256 = "sha256-QHiZPYFwDQzbXVSuhwzQqBRXlkG9QVU+Jl6SKvBoCwQ=";
    };
  };
  kimi-code-darwin-arm64 = {
    pname = "kimi-code-darwin-arm64";
    version = "0.22.1";
    src = fetchurl {
      url = "https://github.com/MoonshotAI/kimi-code/releases/download/@moonshot-ai/kimi-code@0.22.1/kimi-code-darwin-arm64.zip";
      sha256 = "sha256-kTYYdsuVhDpKOq0TFW7dwMrxwH7ZZ2S9/ECdO/fKpMw=";
    };
  };
  kimi-code-darwin-x64 = {
    pname = "kimi-code-darwin-x64";
    version = "0.22.1";
    src = fetchurl {
      url = "https://github.com/MoonshotAI/kimi-code/releases/download/@moonshot-ai/kimi-code@0.22.1/kimi-code-darwin-x64.zip";
      sha256 = "sha256-jgnesSKI/YFrUix4bZNVenRlkufmg6Vw/CBWbuuHnd0=";
    };
  };
  kimi-code-linux-arm64 = {
    pname = "kimi-code-linux-arm64";
    version = "0.22.1";
    src = fetchurl {
      url = "https://github.com/MoonshotAI/kimi-code/releases/download/@moonshot-ai/kimi-code@0.22.1/kimi-code-linux-arm64.zip";
      sha256 = "sha256-BR/mwebAelkIv/PF42rx2zRi2Wdzrtkn+Y9ZhAFCWp8=";
    };
  };
  kimi-code-linux-x64 = {
    pname = "kimi-code-linux-x64";
    version = "0.22.1";
    src = fetchurl {
      url = "https://github.com/MoonshotAI/kimi-code/releases/download/@moonshot-ai/kimi-code@0.22.1/kimi-code-linux-x64.zip";
      sha256 = "sha256-qyCy7rAMgvrqsbgYaJV7Kv08fK/toUjw531sStKnNFA=";
    };
  };
  komari = {
    pname = "komari";
    version = "be6af33d85079394161b4101614004c5b8003d27";
    src = fetchgit {
      url = "https://github.com/komari-monitor/komari.git";
      rev = "be6af33d85079394161b4101614004c5b8003d27";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-MsP3Jj9w9E7h2DdjyV4CV39GXBC5QWe/6DRRA3f16Oo=";
    };
    date = "2026-06-30";
  };
  komari-monitor-rs = {
    pname = "komari-monitor-rs";
    version = "a17a8b2ee5404f769ab28e7079f1b41b4be1cb12";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/komari-monitor-rs.git";
      rev = "a17a8b2ee5404f769ab28e7079f1b41b4be1cb12";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-Bbh0vfAUKS8sf2JR+8N8UmtiL6oQ1VSzAkDNRPrRD0g=";
    };
    date = "2026-05-26";
  };
  komari-web = {
    pname = "komari-web";
    version = "50996de81156028cd96269b6cc89ce9bb37b898d";
    src = fetchgit {
      url = "https://github.com/komari-monitor/komari-web.git";
      rev = "50996de81156028cd96269b6cc89ce9bb37b898d";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-6wDLVoK5B5dDpUm4e2/RrKYrInWmrwHo8cn5L6DwVR0=";
    };
    date = "2026-06-30";
  };
  lazyssh = {
    pname = "lazyssh";
    version = "48995806bf1aa1c9c41bb8dff6b5ddd698539311";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/lazyssh.git";
      rev = "48995806bf1aa1c9c41bb8dff6b5ddd698539311";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-/jywzjLZ51nSbiDbhw3H1eES3p0sZ9MSayGjJFQ+RKM=";
    };
    date = "2025-11-28";
  };
  leaf = {
    pname = "leaf";
    version = "1.25.0";
    src = fetchFromGitHub {
      owner = "rivolink";
      repo = "leaf";
      rev = "1.25.0";
      fetchSubmodules = false;
      sha256 = "sha256-OSx797tkwjKU9j+0AhQIT7uLM75PzHVw12d5LG6vT3Q=";
    };
  };
  llama-cpp-turboquant = {
    pname = "llama-cpp-turboquant";
    version = "08e025c06ab521e4fa9e5c08b80af57614543e53";
    src = fetchgit {
      url = "https://github.com/johndpope/llama-cpp-turboquant.git";
      rev = "08e025c06ab521e4fa9e5c08b80af57614543e53";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-Ve6ddo4JvFX8T5o/iGpKlm5Sa5OKJ/CyrY3MZuMdgC0=";
    };
    date = "2026-05-08";
  };
  lotspeed = {
    pname = "lotspeed";
    version = "3df7cb1e3526526496191edb00e2e4e92e7da761";
    src = fetchgit {
      url = "https://github.com/uk0/lotspeed.git";
      rev = "3df7cb1e3526526496191edb00e2e4e92e7da761";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-Z6m93vi8u3BeQ1SW2sgKVrhHykyp/WTrcJMOaRzFJ38=";
    };
    date = "2026-01-19";
  };
  lucebox-hub = {
    pname = "lucebox-hub";
    version = "f7fc9448c4b006a8a697176d1a557fdf16146561";
    src = fetchgit {
      url = "https://github.com/Luce-Org/lucebox-hub.git";
      rev = "f7fc9448c4b006a8a697176d1a557fdf16146561";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-hpz0EASxtbAFMkG+A/+wq4jJbPaEAvUMDk3LFghvhhA=";
    };
    date = "2026-06-30";
  };
  magiskboot-bin = {
    pname = "magiskboot-bin";
    version = "v30.7";
    src = fetchurl {
      url = "https://github.com/topjohnwu/Magisk/releases/download/v30.7/Magisk-v30.7.apk";
      sha256 = "sha256-4NMtISNTKGD5cSPZJ7G7hsTgjm/YpIv8a1vuCvrp69U=";
    };
  };
  mcp-cli = {
    pname = "mcp-cli";
    version = "v0.3.0";
    src = fetchFromGitHub {
      owner = "philschmid";
      repo = "mcp-cli";
      rev = "v0.3.0";
      fetchSubmodules = false;
      sha256 = "sha256-S924rqlVKzUFD63NDyK5bbXnonra+/UoH6j78AAj3d0=";
    };
  };
  micyou = {
    pname = "micyou";
    version = "v1.3.5";
    src = fetchFromGitHub {
      owner = "LanRhyme";
      repo = "MicYou";
      rev = "v1.3.5";
      fetchSubmodules = false;
      sha256 = "sha256-qefp7gG7DfE/GN81UB981KvS4Q2oIxNLM2/76NLacXs=";
    };
  };
  mihomo-to-uri = {
    pname = "mihomo-to-uri";
    version = "ce092a6b93a191873c4c63beb694654ac38b5e96";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/mihomo-to-uri.git";
      rev = "ce092a6b93a191873c4c63beb694654ac38b5e96";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-+tx2CBZ+ipqUrUoAMsmdj/QpLabSFlcwy4rKsQ8rTio=";
    };
    date = "2026-05-19";
  };
  mlx-optiq = {
    pname = "mlx-optiq";
    version = "0.2.4";
    src = fetchurl {
      url = "https://files.pythonhosted.org/packages/source/m/mlx-optiq/mlx_optiq-0.2.4.tar.gz";
      sha256 = "sha256-3XUYB71Aqqr3jtoiVG3eDsQis7OhC2J8eka18edrnQ0=";
    };
  };
  multica-cli = {
    pname = "multica-cli";
    version = "v0.4.0";
    src = fetchFromGitHub {
      owner = "multica-ai";
      repo = "multica";
      rev = "v0.4.0";
      fetchSubmodules = false;
      sha256 = "sha256-DOcTIhDxVKyXXG/WXMEXNFh81Veg6HaGogxjnYOKw+o=";
    };
  };
  n9router = {
    pname = "n9router";
    version = "f493218f288c0db1951fa6ef01c611781cf9ce10";
    src = fetchgit {
      url = "https://github.com/nightwalker89/n9router.git";
      rev = "f493218f288c0db1951fa6ef01c611781cf9ce10";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-/bJFXLlo3Wd192i49sgvYpw4yxcTUeVMW1YXAcoiErI=";
    };
    date = "2026-06-10";
  };
  nf_deaf = {
    pname = "nf_deaf";
    version = "cf346a3c270e5eb6d31b1d0c110efaf7a8b9193e";
    src = fetchgit {
      url = "https://github.com/kmb21y66/nf_deaf.git";
      rev = "cf346a3c270e5eb6d31b1d0c110efaf7a8b9193e";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-vLcJWJtD7W1D37yFE5fGnZP6ZOKdfKkd+fTte0GDu0g=";
    };
    date = "2026-01-23";
  };
  octopus-api = {
    pname = "octopus-api";
    version = "f79365ca0c84966daeead0e70ece04ed6969dda7";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/octopus.git";
      rev = "f79365ca0c84966daeead0e70ece04ed6969dda7";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-mdVKnhPGHaZVursHVbDed6Q29t5LUtr4Rbw72g0Voaw=";
    };
    date = "2026-06-13";
  };
  okmain = {
    pname = "okmain";
    version = "e51da5fa7efde11dad009592bdf3b41980c3555f";
    src = fetchgit {
      url = "https://github.com/si14/okmain.git";
      rev = "e51da5fa7efde11dad009592bdf3b41980c3555f";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-klCN4sgRvN/9QyXgu0+XBJMpjTOBscNG0ZqM4CV2QBo=";
    };
    date = "2026-03-10";
  };
  oli = {
    pname = "oli";
    version = "32f1cc2b5840718a26b286f8b2b8975401f7e720";
    src = fetchgit {
      url = "https://github.com/apache/opendal-oli.git";
      rev = "32f1cc2b5840718a26b286f8b2b8975401f7e720";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-jsWtl+OchX6tgoyr1xCzZZfpA3UzcCfTzwl7Xc1sojU=";
    };
    date = "2026-06-03";
  };
  omniroute = {
    pname = "omniroute";
    version = "v3.8.42";
    src = fetchFromGitHub {
      owner = "diegosouzapw";
      repo = "OmniRoute";
      rev = "v3.8.42";
      fetchSubmodules = false;
      sha256 = "sha256-D1qAjdNRMVecvOxpk/eE6mpwoa/ktvEFTxKgXJzMH8w=";
    };
  };
  openixcli = {
    pname = "openixcli";
    version = "668d7b1678f8abbffdb4160da69da713cdc026c9";
    src = fetchgit {
      url = "https://github.com/YuzukiTsuru/OpenixCLI.git";
      rev = "668d7b1678f8abbffdb4160da69da713cdc026c9";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-dvvkh6gcN9cUFd7NmNYcW7lI+q2S5vWiyB4GHv6AyMo=";
    };
    date = "2026-06-18";
  };
  oura-cli = {
    pname = "oura-cli";
    version = "5354384a57006ab68ef448f196b48bf1caebb2ea";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/oura-cli.git";
      rev = "5354384a57006ab68ef448f196b48bf1caebb2ea";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-hCkd7yBHND/UxRjPNU7SaG0l8MvnLgFpSgAms1quAeM=";
    };
    date = "2026-01-31";
  };
  pg_duckdb = {
    pname = "pg_duckdb";
    version = "95d5524a78d70c96b58583608257d7ec7b98b449";
    src = fetchgit {
      url = "https://github.com/duckdb/pg_duckdb.git";
      rev = "95d5524a78d70c96b58583608257d7ec7b98b449";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-2wtnSc79m5Rrt40oLqhJxzUEtBTnXU/GQQ+LbH/g3JQ=";
    };
    date = "2026-06-26";
  };
  pi-agent-rust = {
    pname = "pi-agent-rust";
    version = "v0.1.20";
    src = fetchFromGitHub {
      owner = "Dicklesworthstone";
      repo = "pi_agent_rust";
      rev = "v0.1.20";
      fetchSubmodules = false;
      sha256 = "sha256-OWbxlt7+Z+C62YoslJCL9DvxeWs0mCoH4GC3R45/hDA=";
    };
  };
  piko = {
    pname = "piko";
    version = "v0.1.0";
    src = fetchFromGitHub {
      owner = "UruhaLushia";
      repo = "piko";
      rev = "v0.1.0";
      fetchSubmodules = false;
      sha256 = "sha256-lqNVYMhTbiF5lbehPMg1irNzbrOsoXIPu0fSBn+N6F4=";
    };
  };
  planify = {
    pname = "planify";
    version = "v4.19.4";
    src = fetchFromGitHub {
      owner = "alainm23";
      repo = "planify";
      rev = "v4.19.4";
      fetchSubmodules = false;
      sha256 = "sha256-TUtaz0dYy4j6WBcHCLJR5+Rh6PwcU51PfxvdgEUM+vU=";
    };
  };
  polymarket-cli = {
    pname = "polymarket-cli";
    version = "v0.1.5";
    src = fetchurl {
      url = "https://github.com/Polymarket/polymarket-cli/releases/download/v0.1.5/polymarket-v0.1.5-x86_64-unknown-linux-gnu.tar.gz";
      sha256 = "sha256-3ZyMnzlliEJ+/X9ThHSyoRQ0wh/fbKkKp1oO2yvT2KY=";
    };
  };
  polymarket-cli-aarch64-linux = {
    pname = "polymarket-cli-aarch64-linux";
    version = "v0.1.5";
    src = fetchurl {
      url = "https://github.com/Polymarket/polymarket-cli/releases/download/v0.1.5/polymarket-v0.1.5-aarch64-unknown-linux-gnu.tar.gz";
      sha256 = "sha256-GjPJLG/7IRciVEltQA5Ki/aZbNAO6Q4FGZvkdgAFfNo=";
    };
  };
  powermem = {
    pname = "powermem";
    version = "1.1.7";
    src = fetchurl {
      url = "https://github.com/oceanbase/powermem/releases/download/v1.1.7/powermem-1.1.7-linux-amd64-binaries.tar.gz";
      sha256 = "sha256-CmicB7f9t4RJ7QCQVWduaFn6TqcE4CalI3SDOvnl6/g=";
    };
  };
  proxy-ns = {
    pname = "proxy-ns";
    version = "5d08d18f9171d7e65fabad06ee81021d9bca0a21";
    src = fetchgit {
      url = "https://github.com/OkamiW/proxy-ns.git";
      rev = "5d08d18f9171d7e65fabad06ee81021d9bca0a21";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-4w5aiVpbmlGkCyg5YQnjXYkdgrbZADiWw+l4kd6ecek=";
    };
    date = "2026-04-09";
  };
  proxy_pool = {
    pname = "proxy_pool";
    version = "edd637ad8edb83c68134d532e1340a8d3195bb9d";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/proxy_pool.git";
      rev = "edd637ad8edb83c68134d532e1340a8d3195bb9d";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-tdJvMNDUUgbv7l0iaubTsEGQmWiwUo28uC7WLaXjZS8=";
    };
    date = "2026-05-17";
  };
  pymicro-features = {
    pname = "pymicro-features";
    version = "v2.0.2";
    src = fetchFromGitHub {
      owner = "rhasspy";
      repo = "pymicro-features";
      rev = "v2.0.2";
      fetchSubmodules = false;
      sha256 = "sha256-bCvrsvifUxM7jfqBePmcYviZ5pIy1ZApq48Pu/5b7AM=";
    };
  };
  pymicro-wakeword = {
    pname = "pymicro-wakeword";
    version = "v2.4.1";
    src = fetchFromGitHub {
      owner = "OHF-Voice";
      repo = "pymicro-wakeword";
      rev = "v2.4.1";
      fetchSubmodules = false;
      sha256 = "sha256-R8uAKJygh5mo8U9k0uT358RMxJnezRxAATjjaK4D9PI=";
    };
  };
  python-mpv = {
    pname = "python-mpv";
    version = "93c4de9bb7a0e1cfcb9d305a58a326bb24ef9d47";
    src = fetchgit {
      url = "https://github.com/jaseg/python-mpv.git";
      rev = "93c4de9bb7a0e1cfcb9d305a58a326bb24ef9d47";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-fTvEcYJR7gOjmgsQkHIB+VM2rgWZLA6D/Wqa/QTyn9Q=";
    };
    date = "2025-04-25";
  };
  qmdr = {
    pname = "qmdr";
    version = "v1.0.3";
    src = fetchFromGitHub {
      owner = "uf-hy";
      repo = "qmdr";
      rev = "v1.0.3";
      fetchSubmodules = false;
      sha256 = "sha256-hVHdio3IlOU7kJwBXFuKNVTBKf+IpnamTokRPdyQ1Q0=";
    };
  };
  realitlscanner = {
    pname = "realitlscanner";
    version = "v0.2.1";
    src = fetchFromGitHub {
      owner = "XTLS";
      repo = "RealiTLScanner";
      rev = "v0.2.1";
      fetchSubmodules = false;
      sha256 = "sha256-rshU3nBguzBleM9zlPHPFQy0C+C6LYE2EI1uKRl4DN4=";
    };
  };
  rikugan = {
    pname = "rikugan";
    version = "v1.3.2";
    src = fetchFromGitHub {
      owner = "buzzer-re";
      repo = "Rikugan";
      rev = "v1.3.2";
      fetchSubmodules = false;
      sha256 = "sha256-Fw4Ya3rh3PYP9cBq0d+1TW/ekL5II00OPExqUIx57AA=";
    };
  };
  rustconn = {
    pname = "rustconn";
    version = "v0.17.5";
    src = fetchFromGitHub {
      owner = "totoshko88";
      repo = "RustConn";
      rev = "v0.17.5";
      fetchSubmodules = false;
      sha256 = "sha256-gDQnVMuSaJM0RFUySPpF+n0FLZ0yeA5v0Vca4QLtJ8c=";
    };
  };
  selector4nix = {
    pname = "selector4nix";
    version = "v0.7.0";
    src = fetchFromGitHub {
      owner = "StarryReverie";
      repo = "selector4nix";
      rev = "v0.7.0";
      fetchSubmodules = false;
      sha256 = "sha256-yuVXPzbaudYkGA3Vud+L2fFy6onF703MkAUgBVYrcK8=";
    };
  };
  sigmo = {
    pname = "sigmo";
    version = "v4.1.2";
    src = fetchFromGitHub {
      owner = "damonto";
      repo = "sigmo";
      rev = "v4.1.2";
      fetchSubmodules = false;
      sha256 = "sha256-T6+V2W95aelYhGoxnmm+MkE1G82IRJvGpujDEaVXJrg=";
    };
  };
  smali = {
    pname = "smali";
    version = "v2.5.2";
    src = fetchFromGitHub {
      owner = "JesusFreke";
      repo = "smali";
      rev = "v2.5.2";
      fetchSubmodules = false;
      sha256 = "sha256-UkABsX7KaDZyFR4MTmpjnw4dGXChbLB281Kav/k7/00=";
    };
  };
  smartdns-rs = {
    pname = "smartdns-rs";
    version = "303ade885aeee45c2798de7c20a3bcccf48e6e7c";
    src = fetchgit {
      url = "https://github.com/mokeyish/smartdns-rs.git";
      rev = "303ade885aeee45c2798de7c20a3bcccf48e6e7c";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-U1pTM0aXSlXZJ8LfTL3mZJstAKo5GdMyZ9g3omazFfc=";
    };
    date = "2026-04-07";
  };
  sofixer = {
    pname = "sofixer";
    version = "v2.1.7";
    src = fetchFromGitHub {
      owner = "F8LEFT";
      repo = "SoFixer";
      rev = "v2.1.7";
      fetchSubmodules = false;
      sha256 = "sha256-emvN0iNPoADAiH4eOPCIViNHpItKyacySu/9EK2BaeQ=";
    };
  };
  srv-us = {
    pname = "srv-us";
    version = "664ef6a4fab24c5f3f1fc2fa07cc3e0c43d90cf9";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/srv.us.git";
      rev = "664ef6a4fab24c5f3f1fc2fa07cc3e0c43d90cf9";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-CEaSHwR8munYwKkLkCAGGbnRsel+by72QBY28lIRQO4=";
    };
    date = "2026-02-08";
  };
  subs-check = {
    pname = "subs-check";
    version = "87eeee99a1faaa753eac6e4ab24107b9c53c09fe";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/subs-check.git";
      rev = "87eeee99a1faaa753eac6e4ab24107b9c53c09fe";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-espBOu7bFpTAZW1TwpWB7loEUDRLbO0rcJLev/SlZNI=";
    };
    date = "2026-07-05";
  };
  substack-dl = {
    pname = "substack-dl";
    version = "866b2566f9244374e7df5f0f404098c5f81e7b06";
    src = fetchFromGitHub {
      owner = "p3psi-boo";
      repo = "substack-dl";
      rev = "866b2566f9244374e7df5f0f404098c5f81e7b06";
      fetchSubmodules = false;
      sha256 = "sha256-F8497VhGZXCyYd2pHaJKr1WXWKEbUSSQ+3LaA1cnZXs=";
    };
    date = "2022-08-08";
  };
  sugar-wifi-conf = {
    pname = "sugar-wifi-conf";
    version = "2c578f1163f0b6f55ad4d005c5580b8ddcf92bff";
    src = fetchgit {
      url = "https://github.com/PiSugar/sugar-wifi-conf.git";
      rev = "2c578f1163f0b6f55ad4d005c5580b8ddcf92bff";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-3/ElVEjNyusipLLDHuE9vdvjkmPRw2GtKCQwnPxQQx0=";
    };
    date = "2026-06-27";
  };
  tcp-brutal = {
    pname = "tcp-brutal";
    version = "f11e52d88c7ad2285896de018c2d96d4687f0ab6";
    src = fetchgit {
      url = "https://github.com/apernet/tcp-brutal.git";
      rev = "f11e52d88c7ad2285896de018c2d96d4687f0ab6";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-2oi3TrpNllyayzmZeSvYWHuisZOZpfobqxWRqh/zyRY=";
    };
    date = "2026-04-26";
  };
  terminal-use = {
    pname = "terminal-use";
    version = "v1.2.0";
    src = fetchFromGitHub {
      owner = "flipbit03";
      repo = "terminal-use";
      rev = "v1.2.0";
      fetchSubmodules = false;
      sha256 = "sha256-wHu+L//x1NiXTxD2mas0niV/TbTezg4MC7wUWAfgxpY=";
    };
  };
  trusttunnel = {
    pname = "trusttunnel";
    version = "v1.0.33";
    src = fetchFromGitHub {
      owner = "TrustTunnel";
      repo = "TrustTunnel";
      rev = "v1.0.33";
      fetchSubmodules = false;
      sha256 = "sha256-uTAjZbPBCVucjjrl3cYCT6+6cSxiIe/Q3jm+Oa+Fl7E=";
    };
  };
  uber-apk-signer = {
    pname = "uber-apk-signer";
    version = "v1.3.0";
    src = fetchFromGitHub {
      owner = "patrickfav";
      repo = "uber-apk-signer";
      rev = "v1.3.0";
      fetchSubmodules = false;
      sha256 = "sha256-pxGzRAUuYUyqcanoaQ4rqbZQfLMWdmc4GdGBeLw7O+M=";
    };
  };
  usque = {
    pname = "usque";
    version = "0fa6da9e34714a13676361afbfe6a58ad6597110";
    src = fetchgit {
      url = "https://github.com/Diniboy1123/usque.git";
      rev = "0fa6da9e34714a13676361afbfe6a58ad6597110";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-qP7OsjuaSlEjQm1t/EiRcOGdffgc8opdwlQ6xuIslYA=";
    };
    date = "2026-06-21";
  };
  ut = {
    pname = "ut";
    version = "0e19819cf148ae60ae7b1e279ef2e5646bb40f6d";
    src = fetchgit {
      url = "https://github.com/ksdme/ut.git";
      rev = "0e19819cf148ae60ae7b1e279ef2e5646bb40f6d";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-jWI1IUHxKfx9FeTyxdorKk0nTrhqASHODlp3CbET0zk=";
    };
    date = "2026-05-09";
  };
  vanissh = {
    pname = "vanissh";
    version = "c2ee4700d8112b5c6f06fd587b6f11c9aafbdf72";
    src = fetchgit {
      url = "https://github.com/k4yt3x/vanissh.git";
      rev = "c2ee4700d8112b5c6f06fd587b6f11c9aafbdf72";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-5pn2gKiGnd5nfGv3qjaFdvWzMnFjWLUwLtoZb8es7YE=";
    };
    date = "2025-09-21";
  };
  vgpu_unlock-rs = {
    pname = "vgpu_unlock-rs";
    version = "v19.1";
    src = fetchFromGitHub {
      owner = "rbqvq";
      repo = "vgpu_unlock-rs";
      rev = "v19.1";
      fetchSubmodules = false;
      sha256 = "sha256-CdLh8OU/idWLzdPEfQpF5Uost1vaNSEJkBJJlHdPb1c=";
    };
  };
  vibevoice-cpp = {
    pname = "vibevoice-cpp";
    version = "ad856bda6b1311b7f3d7c4a667be43eeb8a8249a";
    src = fetchgit {
      url = "https://github.com/mudler/vibevoice.cpp.git";
      rev = "ad856bda6b1311b7f3d7c4a667be43eeb8a8249a";
      fetchSubmodules = true;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-No0jOFb/NzWOkNzTrFK/O4D6aKJZlDnExfw+cpDWYw0=";
    };
    date = "2026-05-05";
  };
  vja = {
    pname = "vja";
    version = "1bbea5452dae7a7ee97c2b9b3317fc989a4cf3ea";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/vikunja-cli.git";
      rev = "1bbea5452dae7a7ee97c2b9b3317fc989a4cf3ea";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-cDEquvuDHqz2bm9oecHWvTyB2fop6ORgPu8GQ3t2xNE=";
    };
    date = "2026-06-28";
  };
  vohive = {
    pname = "vohive";
    version = "9465ae76ea36067d7feef33aee80ff0a1ec376a0";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/vohive.git";
      rev = "9465ae76ea36067d7feef33aee80ff0a1ec376a0";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-knya7EaLnmeY24/TwAq6XpulvhOYKnpV3gEqy8PMQMM=";
    };
    date = "2026-07-05";
  };
  web-search = {
    pname = "web-search";
    version = "d3be36dc80cc5d8f91df78ddb0d1a018d49d5e70";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/web-search.git";
      rev = "d3be36dc80cc5d8f91df78ddb0d1a018d49d5e70";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-7eTnPf+pb2yFK/qrLP37GI96EqexbWDHqfNhoMYNdJg=";
    };
    date = "2026-03-21";
  };
  wikiteam3 = {
    pname = "wikiteam3";
    version = "06346a4305f8f8c5ced633c3e0dc38b3bb960e6e";
    src = fetchgit {
      url = "https://github.com/saveweb/wikiteam3.git";
      rev = "06346a4305f8f8c5ced633c3e0dc38b3bb960e6e";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-gdUZzfIn9nM60+mYacFblmyqa+pY+EbR4lPZnuy6Amk=";
    };
    date = "2025-12-24";
  };
  wsl-vpnkit = {
    pname = "wsl-vpnkit";
    version = "v0.4.1";
    src = fetchFromGitHub {
      owner = "sakai135";
      repo = "wsl-vpnkit";
      rev = "v0.4.1";
      fetchSubmodules = false;
      sha256 = "sha256-Igbr3L2W32s4uBepllSz07bkbI3qwAKMZkBrXLqGrGA=";
    };
  };
  wtfps = {
    pname = "wtfps";
    version = "44ea48854ac49396f35426246426ffcdf5616631";
    src = fetchgit {
      url = "https://codeberg.org/joelkoen/wtfps.git";
      rev = "44ea48854ac49396f35426246426ffcdf5616631";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-kWN9jOOEY1uW/1VtEYLzhm3Q2RbrjlGETk+RS2v2mTc=";
    };
    date = "2024-10-21";
  };
  wxmp_searxng = {
    pname = "wxmp_searxng";
    version = "f055197fe81897e0cb8bdf37046fe34f7596f346";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/wxmp_searxng.git";
      rev = "f055197fe81897e0cb8bdf37046fe34f7596f346";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-wVbL5R5bv1k4FHinT5M8lW91rUCzCug9M6Zdbzq020M=";
    };
    date = "2026-07-12";
  };
  xurl = {
    pname = "xurl";
    version = "24eedb2ff2b141b025efa1a1658fd7f3e88cdd01";
    src = fetchgit {
      url = "https://github.com/Xuanwo/xurl.git";
      rev = "24eedb2ff2b141b025efa1a1658fd7f3e88cdd01";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-M5GurO0O5LRDvFg6MtDZz40Qjdj3EKvzMiJWhVvwDwg=";
    };
    date = "2026-03-23";
  };
}
