# This file was generated by nvfetcher, please do not modify it manually.
{
  fetchgit,
  fetchurl,
  fetchFromGitHub,
  dockerTools,
}:
{
  bb-browser = {
    pname = "bb-browser";
    version = "bb-browser-v0.11.2";
    src = fetchFromGitHub {
      owner = "epiral";
      repo = "bb-browser";
      rev = "bb-browser-v0.11.2";
      fetchSubmodules = false;
      sha256 = "sha256-DoAd8oN4Il5xGHkcDKgmLczUYj0D0PvxhbSmnDYb+wI=";
    };
  };
  bbdown = {
    pname = "bbdown";
    version = "1b2fbd4372d0b9840d28072f7914ae9887508e5d";
    src = fetchgit {
      url = "https://github.com/nilaoda/BBDown.git";
      rev = "1b2fbd4372d0b9840d28072f7914ae9887508e5d";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-GrBD4PptkryCfpyvnRTPoNfvnT2hAUhZQeS+0SQwJzo=";
    };
    date = "2026-05-14";
  };
  bcut-asr = {
    pname = "bcut-asr";
    version = "07fc48213a415490da1ae40cd8988a087cb02ec6";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/bcut-asr-go.git";
      rev = "07fc48213a415490da1ae40cd8988a087cb02ec6";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-uGhqogSabFpuWQ1h88oXrCOPuJPyfuR9nx8Ug5WbUl0=";
    };
    date = "2026-01-14";
  };
  bichon = {
    pname = "bichon";
    version = "1.4.3";
    src = fetchFromGitHub {
      owner = "rustmailer";
      repo = "bichon";
      rev = "1.4.3";
      fetchSubmodules = false;
      sha256 = "sha256-XOEP429FOrTVd7eUkN/gICPgemTC9/h/NegiVyqTqmA=";
    };
  };
  binsider-bin = {
    pname = "binsider-bin";
    version = "0.3.2";
    src = fetchurl {
      url = "https://github.com/orhun/binsider/releases/download/v0.3.2/binsider-0.3.2-x86_64-unknown-linux-musl.tar.gz";
      sha256 = "sha256-/TScWAgk8rUv981qMCqqd0o/5L7t2TeFFb3+zsDYd6M=";
    };
  };
  binsider-bin-aarch64-linux = {
    pname = "binsider-bin-aarch64-linux";
    version = "0.3.2";
    src = fetchurl {
      url = "https://github.com/orhun/binsider/releases/download/v0.3.2/binsider-0.3.2-aarch64-unknown-linux-musl.tar.gz";
      sha256 = "sha256-pedF8eMfFmfsFPYjm6w8McxQQ5YmJLEtrkGC0xtSROI=";
    };
  };
  binsider-bin-darwin-arm64 = {
    pname = "binsider-bin-darwin-arm64";
    version = "0.3.2";
    src = fetchurl {
      url = "https://github.com/orhun/binsider/releases/download/v0.3.2/binsider-0.3.2-aarch64-apple-darwin.tar.gz";
      sha256 = "sha256-CNYBoXtsEpvWzXGqp2+loZT88hgVzLocm//4tklnT78=";
    };
  };
  binsider-bin-darwin-x64 = {
    pname = "binsider-bin-darwin-x64";
    version = "0.3.2";
    src = fetchurl {
      url = "https://github.com/orhun/binsider/releases/download/v0.3.2/binsider-0.3.2-x86_64-apple-darwin.tar.gz";
      sha256 = "sha256-HDz+JDAf3WeezxZTeLsTNb9kAdAHfPOxu6/lkchG6Bo=";
    };
  };
  bird-cli = {
    pname = "bird-cli";
    version = "c815bfbe3cefd0104adf411cd2148995c31f2ba0";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/bird-fork.git";
      rev = "c815bfbe3cefd0104adf411cd2148995c31f2ba0";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-47N0//wE3G9RGrcSa0JJTnu9mbw/MK58PNnuqYyitvw=";
    };
    date = "2026-03-09";
  };
  browseros = {
    pname = "browseros";
    version = "0.44.0.1";
    src = fetchurl {
      url = "https://github.com/browseros-ai/BrowserOS/releases/download/v0.44.0.1/BrowserOS_v0.44.0.1_x64.AppImage";
      sha256 = "sha256-ALnyVMnexYy48br9qbWaEbOZm7hJR9g39a9nYzbWXwo=";
    };
  };
  classyshark = {
    pname = "classyshark";
    version = "8.2";
    src = fetchFromGitHub {
      owner = "google";
      repo = "android-classyshark";
      rev = "8.2";
      fetchSubmodules = false;
      sha256 = "sha256-9gENYNWfbPK2jpxiHOf8ETJwXqjFMgplaHNkO3ohhDQ=";
    };
  };
  clihub = {
    pname = "clihub";
    version = "v0.0.8";
    src = fetchFromGitHub {
      owner = "thellimist";
      repo = "clihub";
      rev = "v0.0.8";
      fetchSubmodules = false;
      sha256 = "sha256-Y/O2XmhMYUCV269g+h+DsQT80yVR0ot9RIS+mBJP7BA=";
    };
  };
  clipboard-sync = {
    pname = "clipboard-sync";
    version = "0.2.0";
    src = fetchFromGitHub {
      owner = "dnut";
      repo = "clipboard-sync";
      rev = "0.2.0";
      fetchSubmodules = false;
      sha256 = "sha256-gme5pwQrwQbk8MroF/mGYqlY6hcjM5cHKHL7Y3nlW9k=";
    };
  };
  cliproxyapi = {
    pname = "cliproxyapi";
    version = "v7.1.37";
    src = fetchFromGitHub {
      owner = "router-for-me";
      repo = "CLIProxyAPI";
      rev = "v7.1.37";
      fetchSubmodules = false;
      sha256 = "sha256-5D5QvhIUh71oJjp2fBv/fpFV2zLl3RBdtvspxbuqCqc=";
    };
  };
  cocoindex-code = {
    pname = "cocoindex-code";
    version = "0.2.33";
    src = fetchurl {
      url = "https://github.com/cocoindex-io/cocoindex-code/archive/refs/tags/v0.2.33.tar.gz";
      sha256 = "sha256-zRHuoEw1Vx5LBnE+K+JLYxSOwF2Vyd4Kyd4sypJd84A=";
    };
  };
  coe = {
    pname = "coe";
    version = "c326261675a2455e6dd8c22576681e91644600ef";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/coe.git";
      rev = "c326261675a2455e6dd8c22576681e91644600ef";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-0VL8NsKi1zl8fWJHn9khUwb0Jx0NIEvAdqe7slSfPyw=";
    };
    date = "2026-04-23";
  };
  commandcode2api = {
    pname = "commandcode2api";
    version = "8ea3fd40c8a910b4c042f84440eae02cc2abdd1c";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/commandcode2api.git";
      rev = "8ea3fd40c8a910b4c042f84440eae02cc2abdd1c";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-dXu5+6+yeraWxy8SfR08QQTBSSfic/J20V41ujhKDxc=";
    };
    date = "2026-05-25";
  };
  cookiecloud-rs = {
    pname = "cookiecloud-rs";
    version = "f8a023f42951c8ba97261d3b16e3a4ff53c91f4a";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/CookieCloud.git";
      rev = "f8a023f42951c8ba97261d3b16e3a4ff53c91f4a";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-ak78lDOAaDWCk9dNpQ9nNObwXrhj71umhzu01hagt2I=";
    };
    date = "2026-04-14";
  };
  copilot-api-plus = {
    pname = "copilot-api-plus";
    version = "33eab1d693d9d0997c284b737549495965d3250e";
    src = fetchgit {
      url = "https://github.com/imbuxiangnan-cyber/copilot-api-plus.git";
      rev = "33eab1d693d9d0997c284b737549495965d3250e";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-JdUglXB9oowhZS6482e9HklJESsj01st0QPw0VzNzVw=";
    };
    date = "2026-05-29";
  };
  dagu = {
    pname = "dagu";
    version = "v2.7.7";
    src = fetchFromGitHub {
      owner = "dagu-org";
      repo = "dagu";
      rev = "v2.7.7";
      fetchSubmodules = false;
      sha256 = "sha256-NISU0lCJCqj1gGMffClLxlopjrQ2adK+q0W/c6TIdC0=";
    };
  };
  dailyhot-api = {
    pname = "dailyhot-api";
    version = "v2.0.8";
    src = fetchFromGitHub {
      owner = "imsyy";
      repo = "DailyHotApi";
      rev = "v2.0.8";
      fetchSubmodules = false;
      sha256 = "sha256-dvQ+jnMbNVLpRSX+TDolDqdiWNXdzxGL3HiQEFy6YlA=";
    };
  };
  dalvikus = {
    pname = "dalvikus";
    version = "1.0.13";
    src = fetchurl {
      url = "https://github.com/loerting/dalvikus/archive/refs/tags/v1.0.13.tar.gz";
      sha256 = "sha256-PMSalWlMDIqMWgPZ6Ha7L6TA2VOX6svCAOYMSpzIqHY=";
    };
  };
  dbhub = {
    pname = "dbhub";
    version = "0.21.2";
    src = fetchurl {
      url = "https://registry.npmjs.org/@bytebase/dbhub/-/dbhub-0.21.2.tgz";
      sha256 = "sha256-eOwzRCZAYypW/GY1zXIqusads2xEv50s01xhfroZPTo=";
    };
  };
  deepseek-ocr-rs = {
    pname = "deepseek-ocr-rs";
    version = "02b933df24f5658d10b37dd48c9c354d95c530c3";
    src = fetchgit {
      url = "https://github.com/TimmyOVO/deepseek-ocr.rs.git";
      rev = "02b933df24f5658d10b37dd48c9c354d95c530c3";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-A15gHXKIi/EieK1JpYLedUd63jCfh19S+1e2TS2cdjQ=";
    };
    date = "2026-02-12";
  };
  ecloud-computer-auto-boot = {
    pname = "ecloud-computer-auto-boot";
    version = "1.0.0";
    src = fetchFromGitHub {
      owner = "Samler-Lee";
      repo = "ecloud_computer_auto_boot";
      rev = "1.0.0";
      fetchSubmodules = false;
      sha256 = "sha256-9H11Jli1Ls8FNtvfgt5X+1yEw4zcrqcmBWPdyq8TIf8=";
    };
  };
  einat = {
    pname = "einat";
    version = "b226ff98856e59be7f091bc7d81ceba8ac38bd19";
    src = fetchgit {
      url = "https://github.com/EHfive/einat-ebpf.git";
      rev = "b226ff98856e59be7f091bc7d81ceba8ac38bd19";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-P0Q8DdeEns5xcALj5vIyubc1AE073nozbNoTzRso37w=";
    };
    date = "2025-12-11";
  };
  fdb = {
    pname = "fdb";
    version = "v1.9.0";
    src = fetchFromGitHub {
      owner = "andrzejchm";
      repo = "fdb";
      rev = "v1.9.0";
      fetchSubmodules = false;
      sha256 = "sha256-2DRMbYHf8dVTiEmggcPRh1oaDhNlxqB+bH9QhKpNi0Q=";
    };
  };
  firm = {
    pname = "firm";
    version = "4144d4a08bdf285c09ed2b9781e5ec448004e506";
    src = fetchgit {
      url = "https://github.com/42futures/firm.git";
      rev = "4144d4a08bdf285c09ed2b9781e5ec448004e506";
      fetchSubmodules = true;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-PgFYlw9XNxcPlRXLIBwfVV+geu3NZ9y2+puN6FczIfg=";
    };
    date = "2026-03-11";
  };
  fish-ai = {
    pname = "fish-ai";
    version = "v2.13.1";
    src = fetchFromGitHub {
      owner = "Realiserad";
      repo = "fish-ai";
      rev = "v2.13.1";
      fetchSubmodules = false;
      sha256 = "sha256-ZSFoE9/UesA6GVSYyRKAfj7+uw1gTZ/E08zAHPizAAQ=";
    };
  };
  focaltech-fingerprint = {
    pname = "focaltech-fingerprint";
    version = "d4fbbf901aff44b92d4fa212d9b99e43cda00563";
    src = fetchgit {
      url = "https://github.com/vobademi/FTEXX00-Ubuntu.git";
      rev = "d4fbbf901aff44b92d4fa212d9b99e43cda00563";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-0mnXEQjSWUrz8uSwaeWH6uLLZtGnSUx5OPBxjPNxFTs=";
    };
    date = "2025-10-21";
  };
  fusion = {
    pname = "fusion";
    version = "ecc44eeff94dd4836eed6490a497989194effa2b";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/fusion.git";
      rev = "ecc44eeff94dd4836eed6490a497989194effa2b";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-O9NMromcyvcF97Kb084vrHPjqRgyRqq14Ay8pdsj9rQ=";
    };
    date = "2026-04-12";
  };
  gemma-cpp = {
    pname = "gemma-cpp";
    version = "e93b7c56f0d6f3714250482b6a91a027350d0f78";
    src = fetchgit {
      url = "https://github.com/test1111111111111112/llama-cpp-turboquant-gemma4.git";
      rev = "e93b7c56f0d6f3714250482b6a91a027350d0f78";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-ykzroJr8zXnLe6Wq279/+ltTxbeSEvcbTufz2EyqYqQ=";
    };
    date = "2026-04-04";
  };
  grdpwasm = {
    pname = "grdpwasm";
    version = "a86c64fc9ead223a2f41ee46c39d1edc482cdb13";
    src = fetchgit {
      url = "https://github.com/nakagami/grdpwasm.git";
      rev = "a86c64fc9ead223a2f41ee46c39d1edc482cdb13";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-mSKem4Eb8WZXLeX1e6qgYeyNDtalD+9aqLK8NweMo1E=";
    };
    date = "2026-05-24";
  };
  grok2api = {
    pname = "grok2api";
    version = "64a71f16e02ef100f64256c61923047621d45270";
    src = fetchgit {
      url = "https://github.com/chenyme/grok2api.git";
      rev = "64a71f16e02ef100f64256c61923047621d45270";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-42z45WpgmCJclLs1tvckwvs6CWO4neRjRnUOLhEOsoQ=";
    };
    date = "2026-04-28";
  };
  harlequin-mysql = {
    pname = "harlequin-mysql";
    version = "v1.3.0";
    src = fetchFromGitHub {
      owner = "tconbeer";
      repo = "harlequin-mysql";
      rev = "v1.3.0";
      fetchSubmodules = false;
      sha256 = "sha256-yWPysa0+GYq4b6WpjdyONTev5CbORsAdQ2JjZuumZv4=";
    };
  };
  hev-socks5-server = {
    pname = "hev-socks5-server";
    version = "2.12.0";
    src = fetchgit {
      url = "https://github.com/heiher/hev-socks5-server.git";
      rev = "2.12.0";
      fetchSubmodules = true;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-8U6NXHiL3ZFb+HuBPm4RKLyKn8YNOQg7DJivsOv0Y5w=";
    };
  };
  ida-mcp = {
    pname = "ida-mcp";
    version = "9.3.24";
    src = fetchurl {
      url = "https://github.com/blacktop/ida-mcp-rs/releases/download/v9.3.24/ida-mcp_9.3.24_Linux_x86_64.tar.gz";
      sha256 = "sha256-Ls9ZaPWTFA82waOtndF4lN/ZDC9P1PfEGJxMkhXqNiQ=";
    };
  };
  ida-pro-mcp = {
    pname = "ida-pro-mcp";
    version = "7886e460f61f22839ecc5d6a85c7c1b378567cd3";
    src = fetchgit {
      url = "https://github.com/QiuChenly/ida-pro-mcp-enhancement.git";
      rev = "7886e460f61f22839ecc5d6a85c7c1b378567cd3";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-oNXWKDTEI/2iIwnRBrOHNUW6USYFB7KAF2krv+OIQoA=";
    };
    date = "2026-05-23";
  };
  il2cppdumper = {
    pname = "il2cppdumper";
    version = "v6.7.46";
    src = fetchFromGitHub {
      owner = "Perfare";
      repo = "Il2CppDumper";
      rev = "v6.7.46";
      fetchSubmodules = false;
      sha256 = "sha256-pMxxwBpuZ2EuP7O99XTJcnS3Dq8MdxvUGQHJ0U3bnTY=";
    };
  };
  inet-speed-cli = {
    pname = "inet-speed-cli";
    version = "v1.1.2";
    src = fetchFromGitHub {
      owner = "tsosunchia";
      repo = "iNetSpeed-CLI";
      rev = "v1.1.2";
      fetchSubmodules = false;
      sha256 = "sha256-xAXYhvN7rdXvA2EWsNXRfXlIrumyPluAcmVbMY/pZxE=";
    };
  };
  jarm = {
    pname = "jarm";
    version = "dcd6ee2d5d9a819fe2200f7a6c1f478b896a2f5d";
    src = fetchgit {
      url = "https://github.com/salesforce/jarm.git";
      rev = "dcd6ee2d5d9a819fe2200f7a6c1f478b896a2f5d";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-2Xy4osJOSrrc/UkEifeQTQejcJ02szNzE3fMO5PzQz8=";
    };
    date = "2021-10-12";
  };
  jd-gui = {
    pname = "jd-gui";
    version = "v1.6.6";
    src = fetchFromGitHub {
      owner = "java-decompiler";
      repo = "jd-gui";
      rev = "v1.6.6";
      fetchSubmodules = false;
      sha256 = "sha256-QHiZPYFwDQzbXVSuhwzQqBRXlkG9QVU+Jl6SKvBoCwQ=";
    };
  };
  komari = {
    pname = "komari";
    version = "fb0b9b991058aa35e732871d0a88d32a94c983c5";
    src = fetchgit {
      url = "https://github.com/komari-monitor/komari.git";
      rev = "fb0b9b991058aa35e732871d0a88d32a94c983c5";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-UeV+h0g/zGKYtgLMEOutw2XWf14FXGGRUT2nKQCFyc4=";
    };
    date = "2026-05-31";
  };
  komari-monitor-rs = {
    pname = "komari-monitor-rs";
    version = "a17a8b2ee5404f769ab28e7079f1b41b4be1cb12";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/komari-monitor-rs.git";
      rev = "a17a8b2ee5404f769ab28e7079f1b41b4be1cb12";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-Bbh0vfAUKS8sf2JR+8N8UmtiL6oQ1VSzAkDNRPrRD0g=";
    };
    date = "2026-05-26";
  };
  komari-web = {
    pname = "komari-web";
    version = "4f1ea05bf7ac3579731080ee860550433f600a2d";
    src = fetchgit {
      url = "https://github.com/komari-monitor/komari-web.git";
      rev = "4f1ea05bf7ac3579731080ee860550433f600a2d";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-CA1z7czYD1iFvwYNIUPsQ+9dZaZJhaSJiJj7OZGNKe4=";
    };
    date = "2026-05-31";
  };
  lazyssh = {
    pname = "lazyssh";
    version = "48995806bf1aa1c9c41bb8dff6b5ddd698539311";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/lazyssh.git";
      rev = "48995806bf1aa1c9c41bb8dff6b5ddd698539311";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-/jywzjLZ51nSbiDbhw3H1eES3p0sZ9MSayGjJFQ+RKM=";
    };
    date = "2025-11-28";
  };
  leaf = {
    pname = "leaf";
    version = "1.24.0";
    src = fetchFromGitHub {
      owner = "rivolink";
      repo = "leaf";
      rev = "1.24.0";
      fetchSubmodules = false;
      sha256 = "sha256-wqp2VUBqYFk9eLrVuNqmUyEPINYhuWf5Iq8Ye0oicEA=";
    };
  };
  llama-cpp-turboquant = {
    pname = "llama-cpp-turboquant";
    version = "08e025c06ab521e4fa9e5c08b80af57614543e53";
    src = fetchgit {
      url = "https://github.com/johndpope/llama-cpp-turboquant.git";
      rev = "08e025c06ab521e4fa9e5c08b80af57614543e53";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-Ve6ddo4JvFX8T5o/iGpKlm5Sa5OKJ/CyrY3MZuMdgC0=";
    };
    date = "2026-05-08";
  };
  lotspeed = {
    pname = "lotspeed";
    version = "3df7cb1e3526526496191edb00e2e4e92e7da761";
    src = fetchgit {
      url = "https://github.com/uk0/lotspeed.git";
      rev = "3df7cb1e3526526496191edb00e2e4e92e7da761";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-Z6m93vi8u3BeQ1SW2sgKVrhHykyp/WTrcJMOaRzFJ38=";
    };
    date = "2026-01-19";
  };
  lucebox-hub = {
    pname = "lucebox-hub";
    version = "8305b6c2b10b303e71383bd9777a9664370976c0";
    src = fetchgit {
      url = "https://github.com/Luce-Org/lucebox-hub.git";
      rev = "8305b6c2b10b303e71383bd9777a9664370976c0";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-Wu6LUdSPQyVnoG3l5dKY5ddBYwm1wfGLnCPq5kP4t1U=";
    };
    date = "2026-05-31";
  };
  magiskboot-bin = {
    pname = "magiskboot-bin";
    version = "v30.7";
    src = fetchurl {
      url = "https://github.com/topjohnwu/Magisk/releases/download/v30.7/Magisk-v30.7.apk";
      sha256 = "sha256-4NMtISNTKGD5cSPZJ7G7hsTgjm/YpIv8a1vuCvrp69U=";
    };
  };
  mcp-cli = {
    pname = "mcp-cli";
    version = "v0.3.0";
    src = fetchFromGitHub {
      owner = "philschmid";
      repo = "mcp-cli";
      rev = "v0.3.0";
      fetchSubmodules = false;
      sha256 = "sha256-S924rqlVKzUFD63NDyK5bbXnonra+/UoH6j78AAj3d0=";
    };
  };
  micyou = {
    pname = "micyou";
    version = "v1.3.4hotfix";
    src = fetchFromGitHub {
      owner = "LanRhyme";
      repo = "MicYou";
      rev = "v1.3.4hotfix";
      fetchSubmodules = false;
      sha256 = "sha256-6JyfNrhNhdeX5F3Hy132ZITpmqQsFYJ1Uwx3EVkij9Q=";
    };
  };
  mihomo-to-uri = {
    pname = "mihomo-to-uri";
    version = "ce092a6b93a191873c4c63beb694654ac38b5e96";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/mihomo-to-uri.git";
      rev = "ce092a6b93a191873c4c63beb694654ac38b5e96";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-+tx2CBZ+ipqUrUoAMsmdj/QpLabSFlcwy4rKsQ8rTio=";
    };
    date = "2026-05-19";
  };
  mobilepi = {
    pname = "mobilepi";
    version = "04f4a4250e9cb1fef2ce2753ac49b397da40bb3e";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/MobilePi.git";
      rev = "04f4a4250e9cb1fef2ce2753ac49b397da40bb3e";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-Zd2TzUlkXWCt4LTUjccDIZ4BrZrs3ma2CRsSwKg4K6s=";
    };
    date = "2026-06-01";
  };
  n9router = {
    pname = "n9router";
    version = "9172a4784dcaf08a16479503dd19bf0083cd3a72";
    src = fetchgit {
      url = "https://github.com/nightwalker89/n9router.git";
      rev = "9172a4784dcaf08a16479503dd19bf0083cd3a72";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-zES1ZGGT8/6Q+KfzAtBvqWpNntoHytsqLVsfqwwmq7U=";
    };
    date = "2026-05-31";
  };
  nf_deaf = {
    pname = "nf_deaf";
    version = "cf346a3c270e5eb6d31b1d0c110efaf7a8b9193e";
    src = fetchgit {
      url = "https://github.com/kmb21y66/nf_deaf.git";
      rev = "cf346a3c270e5eb6d31b1d0c110efaf7a8b9193e";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-vLcJWJtD7W1D37yFE5fGnZP6ZOKdfKkd+fTte0GDu0g=";
    };
    date = "2026-01-23";
  };
  octopus-api = {
    pname = "octopus-api";
    version = "8e45d8b0bce6f0d8808f6cd7f979e17ef1ab1b0a";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/octopus.git";
      rev = "8e45d8b0bce6f0d8808f6cd7f979e17ef1ab1b0a";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-qzBa/c6DKElxStPnLBNwskuptLG3aAXwyqexzu5a/uw=";
    };
    date = "2026-05-30";
  };
  okmain = {
    pname = "okmain";
    version = "e51da5fa7efde11dad009592bdf3b41980c3555f";
    src = fetchgit {
      url = "https://github.com/si14/okmain.git";
      rev = "e51da5fa7efde11dad009592bdf3b41980c3555f";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-klCN4sgRvN/9QyXgu0+XBJMpjTOBscNG0ZqM4CV2QBo=";
    };
    date = "2026-03-10";
  };
  oli = {
    pname = "oli";
    version = "fd6e9563272b49d23129bdc72bfdd7a2d73185b5";
    src = fetchgit {
      url = "https://github.com/apache/opendal-oli.git";
      rev = "fd6e9563272b49d23129bdc72bfdd7a2d73185b5";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-qd1/AQQaYMtCF/aFm8blyrXHhffsxRXqCwFdgCSa85Q=";
    };
    date = "2026-05-08";
  };
  openalice = {
    pname = "openalice";
    version = "v0.30.0-beta.1";
    src = fetchFromGitHub {
      owner = "TraderAlice";
      repo = "OpenAlice";
      rev = "v0.30.0-beta.1";
      fetchSubmodules = false;
      sha256 = "sha256-Sr6iW5LCxBdxp7Ia1jmIUIiNAQ3CIh+1zdXTP5+U+iw=";
    };
  };
  openixcli = {
    pname = "openixcli";
    version = "f0ce24a36a4ffdf4be5d22bcd1f83abd330189d9";
    src = fetchgit {
      url = "https://github.com/YuzukiTsuru/OpenixCLI.git";
      rev = "f0ce24a36a4ffdf4be5d22bcd1f83abd330189d9";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-VtyAk8GldFH6UZFKIJ2RrSAtR0PT1pIGTFIMO9r0eUc=";
    };
    date = "2026-05-30";
  };
  oura-cli = {
    pname = "oura-cli";
    version = "5354384a57006ab68ef448f196b48bf1caebb2ea";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/oura-cli.git";
      rev = "5354384a57006ab68ef448f196b48bf1caebb2ea";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-hCkd7yBHND/UxRjPNU7SaG0l8MvnLgFpSgAms1quAeM=";
    };
    date = "2026-01-31";
  };
  paperclip = {
    pname = "paperclip";
    version = "9f8636cf49c6886ce2487c7683aae6acaf4e8126";
    src = fetchgit {
      url = "https://github.com/paperclipai/paperclip.git";
      rev = "9f8636cf49c6886ce2487c7683aae6acaf4e8126";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-Szi92aO1rydPjbvmxGNbNto+QKL3z4/4TNrN86SpSkk=";
    };
    date = "2026-05-31";
  };
  pg_duckdb = {
    pname = "pg_duckdb";
    version = "318ee16cd8c647daa89198a675039726626f44b9";
    src = fetchgit {
      url = "https://github.com/duckdb/pg_duckdb.git";
      rev = "318ee16cd8c647daa89198a675039726626f44b9";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-8S2uS6PgjxVzQA1gYVrEeZjnACXt3ADneDe1MlRNb/g=";
    };
    date = "2026-03-16";
  };
  planify = {
    pname = "planify";
    version = "v4.19.4";
    src = fetchFromGitHub {
      owner = "alainm23";
      repo = "planify";
      rev = "v4.19.4";
      fetchSubmodules = false;
      sha256 = "sha256-TUtaz0dYy4j6WBcHCLJR5+Rh6PwcU51PfxvdgEUM+vU=";
    };
  };
  polymarket-cli = {
    pname = "polymarket-cli";
    version = "v0.1.5";
    src = fetchurl {
      url = "https://github.com/Polymarket/polymarket-cli/releases/download/v0.1.5/polymarket-v0.1.5-x86_64-unknown-linux-gnu.tar.gz";
      sha256 = "sha256-3ZyMnzlliEJ+/X9ThHSyoRQ0wh/fbKkKp1oO2yvT2KY=";
    };
  };
  polymarket-cli-aarch64-linux = {
    pname = "polymarket-cli-aarch64-linux";
    version = "v0.1.5";
    src = fetchurl {
      url = "https://github.com/Polymarket/polymarket-cli/releases/download/v0.1.5/polymarket-v0.1.5-aarch64-unknown-linux-gnu.tar.gz";
      sha256 = "sha256-GjPJLG/7IRciVEltQA5Ki/aZbNAO6Q4FGZvkdgAFfNo=";
    };
  };
  proxy-ns = {
    pname = "proxy-ns";
    version = "5d08d18f9171d7e65fabad06ee81021d9bca0a21";
    src = fetchgit {
      url = "https://github.com/OkamiW/proxy-ns.git";
      rev = "5d08d18f9171d7e65fabad06ee81021d9bca0a21";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-4w5aiVpbmlGkCyg5YQnjXYkdgrbZADiWw+l4kd6ecek=";
    };
    date = "2026-04-09";
  };
  proxy_pool = {
    pname = "proxy_pool";
    version = "edd637ad8edb83c68134d532e1340a8d3195bb9d";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/proxy_pool.git";
      rev = "edd637ad8edb83c68134d532e1340a8d3195bb9d";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-tdJvMNDUUgbv7l0iaubTsEGQmWiwUo28uC7WLaXjZS8=";
    };
    date = "2026-05-17";
  };
  pymicro-features = {
    pname = "pymicro-features";
    version = "v2.0.2";
    src = fetchFromGitHub {
      owner = "rhasspy";
      repo = "pymicro-features";
      rev = "v2.0.2";
      fetchSubmodules = false;
      sha256 = "sha256-bCvrsvifUxM7jfqBePmcYviZ5pIy1ZApq48Pu/5b7AM=";
    };
  };
  pymicro-wakeword = {
    pname = "pymicro-wakeword";
    version = "v2.2.1";
    src = fetchFromGitHub {
      owner = "OHF-Voice";
      repo = "pymicro-wakeword";
      rev = "v2.2.1";
      fetchSubmodules = false;
      sha256 = "sha256-PLGwpmmiLwpDn+Dh5HhaE2w7hJLC0xK/wsuhxxR8rwU=";
    };
  };
  python-mpv = {
    pname = "python-mpv";
    version = "93c4de9bb7a0e1cfcb9d305a58a326bb24ef9d47";
    src = fetchgit {
      url = "https://github.com/jaseg/python-mpv.git";
      rev = "93c4de9bb7a0e1cfcb9d305a58a326bb24ef9d47";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-fTvEcYJR7gOjmgsQkHIB+VM2rgWZLA6D/Wqa/QTyn9Q=";
    };
    date = "2025-04-25";
  };
  qmdr = {
    pname = "qmdr";
    version = "v1.0.3";
    src = fetchFromGitHub {
      owner = "uf-hy";
      repo = "qmdr";
      rev = "v1.0.3";
      fetchSubmodules = false;
      sha256 = "sha256-hVHdio3IlOU7kJwBXFuKNVTBKf+IpnamTokRPdyQ1Q0=";
    };
  };
  realitlscanner = {
    pname = "realitlscanner";
    version = "v0.2.1";
    src = fetchFromGitHub {
      owner = "XTLS";
      repo = "RealiTLScanner";
      rev = "v0.2.1";
      fetchSubmodules = false;
      sha256 = "sha256-rshU3nBguzBleM9zlPHPFQy0C+C6LYE2EI1uKRl4DN4=";
    };
  };
  rikugan = {
    pname = "rikugan";
    version = "v1.3.1";
    src = fetchFromGitHub {
      owner = "buzzer-re";
      repo = "Rikugan";
      rev = "v1.3.1";
      fetchSubmodules = false;
      sha256 = "sha256-xYPk6HWo1nfpKP2piEDuuX2fvcFmiZ8e6NgZPfuHNNI=";
    };
  };
  rustconn = {
    pname = "rustconn";
    version = "v0.15.4";
    src = fetchFromGitHub {
      owner = "totoshko88";
      repo = "RustConn";
      rev = "v0.15.4";
      fetchSubmodules = false;
      sha256 = "sha256-m/nrzP7NJmDazSL0h7vGfTcVTDhhO4nyWe/RORDdvPA=";
    };
  };
  selector4nix = {
    pname = "selector4nix";
    version = "v0.5.0";
    src = fetchFromGitHub {
      owner = "StarryReverie";
      repo = "selector4nix";
      rev = "v0.5.0";
      fetchSubmodules = false;
      sha256 = "sha256-7OH+5mWUwmWFAdnz84VDo9OafQY4IhR+qMEygwFK1Y0=";
    };
  };
  sigmo = {
    pname = "sigmo";
    version = "v4.1.2";
    src = fetchFromGitHub {
      owner = "damonto";
      repo = "sigmo";
      rev = "v4.1.2";
      fetchSubmodules = false;
      sha256 = "sha256-T6+V2W95aelYhGoxnmm+MkE1G82IRJvGpujDEaVXJrg=";
    };
  };
  smali = {
    pname = "smali";
    version = "v2.5.2";
    src = fetchFromGitHub {
      owner = "JesusFreke";
      repo = "smali";
      rev = "v2.5.2";
      fetchSubmodules = false;
      sha256 = "sha256-UkABsX7KaDZyFR4MTmpjnw4dGXChbLB281Kav/k7/00=";
    };
  };
  smartdns-rs = {
    pname = "smartdns-rs";
    version = "303ade885aeee45c2798de7c20a3bcccf48e6e7c";
    src = fetchgit {
      url = "https://github.com/mokeyish/smartdns-rs.git";
      rev = "303ade885aeee45c2798de7c20a3bcccf48e6e7c";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-U1pTM0aXSlXZJ8LfTL3mZJstAKo5GdMyZ9g3omazFfc=";
    };
    date = "2026-04-07";
  };
  sofixer = {
    pname = "sofixer";
    version = "v2.1.7";
    src = fetchFromGitHub {
      owner = "F8LEFT";
      repo = "SoFixer";
      rev = "v2.1.7";
      fetchSubmodules = false;
      sha256 = "sha256-emvN0iNPoADAiH4eOPCIViNHpItKyacySu/9EK2BaeQ=";
    };
  };
  srv-us = {
    pname = "srv-us";
    version = "664ef6a4fab24c5f3f1fc2fa07cc3e0c43d90cf9";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/srv.us.git";
      rev = "664ef6a4fab24c5f3f1fc2fa07cc3e0c43d90cf9";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-CEaSHwR8munYwKkLkCAGGbnRsel+by72QBY28lIRQO4=";
    };
    date = "2026-02-08";
  };
  subs-check = {
    pname = "subs-check";
    version = "f4dbe04587215291496c1ea5215bdaae167c7715";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/subs-check.git";
      rev = "f4dbe04587215291496c1ea5215bdaae167c7715";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-KtK3G3MjOWDezIjfYbmkMOHdYIz+IIDe0gHy49VMYbI=";
    };
    date = "2026-05-09";
  };
  substack-dl = {
    pname = "substack-dl";
    version = "866b2566f9244374e7df5f0f404098c5f81e7b06";
    src = fetchFromGitHub {
      owner = "p3psi-boo";
      repo = "substack-dl";
      rev = "866b2566f9244374e7df5f0f404098c5f81e7b06";
      fetchSubmodules = false;
      sha256 = "sha256-F8497VhGZXCyYd2pHaJKr1WXWKEbUSSQ+3LaA1cnZXs=";
    };
    date = "2022-08-08";
  };
  tcp-brutal = {
    pname = "tcp-brutal";
    version = "f11e52d88c7ad2285896de018c2d96d4687f0ab6";
    src = fetchgit {
      url = "https://github.com/apernet/tcp-brutal.git";
      rev = "f11e52d88c7ad2285896de018c2d96d4687f0ab6";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-2oi3TrpNllyayzmZeSvYWHuisZOZpfobqxWRqh/zyRY=";
    };
    date = "2026-04-26";
  };
  trusttunnel = {
    pname = "trusttunnel";
    version = "v1.0.33";
    src = fetchFromGitHub {
      owner = "TrustTunnel";
      repo = "TrustTunnel";
      rev = "v1.0.33";
      fetchSubmodules = false;
      sha256 = "sha256-uTAjZbPBCVucjjrl3cYCT6+6cSxiIe/Q3jm+Oa+Fl7E=";
    };
  };
  uber-apk-signer = {
    pname = "uber-apk-signer";
    version = "v1.3.0";
    src = fetchFromGitHub {
      owner = "patrickfav";
      repo = "uber-apk-signer";
      rev = "v1.3.0";
      fetchSubmodules = false;
      sha256 = "sha256-pxGzRAUuYUyqcanoaQ4rqbZQfLMWdmc4GdGBeLw7O+M=";
    };
  };
  usque = {
    pname = "usque";
    version = "827ebf177da2dbcbf20448305c19e45e21e30c3b";
    src = fetchgit {
      url = "https://github.com/Diniboy1123/usque.git";
      rev = "827ebf177da2dbcbf20448305c19e45e21e30c3b";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-+iuVCjwpngpcCSZBlhCLebI5J/wKgZ6TserCsK2xL4U=";
    };
    date = "2026-05-30";
  };
  ut = {
    pname = "ut";
    version = "0e19819cf148ae60ae7b1e279ef2e5646bb40f6d";
    src = fetchgit {
      url = "https://github.com/ksdme/ut.git";
      rev = "0e19819cf148ae60ae7b1e279ef2e5646bb40f6d";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-jWI1IUHxKfx9FeTyxdorKk0nTrhqASHODlp3CbET0zk=";
    };
    date = "2026-05-09";
  };
  vanissh = {
    pname = "vanissh";
    version = "c2ee4700d8112b5c6f06fd587b6f11c9aafbdf72";
    src = fetchgit {
      url = "https://github.com/k4yt3x/vanissh.git";
      rev = "c2ee4700d8112b5c6f06fd587b6f11c9aafbdf72";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-5pn2gKiGnd5nfGv3qjaFdvWzMnFjWLUwLtoZb8es7YE=";
    };
    date = "2025-09-21";
  };
  vgpu_unlock-rs = {
    pname = "vgpu_unlock-rs";
    version = "v19.1";
    src = fetchFromGitHub {
      owner = "rbqvq";
      repo = "vgpu_unlock-rs";
      rev = "v19.1";
      fetchSubmodules = false;
      sha256 = "sha256-CdLh8OU/idWLzdPEfQpF5Uost1vaNSEJkBJJlHdPb1c=";
    };
  };
  vibevoice-cpp = {
    pname = "vibevoice-cpp";
    version = "ad856bda6b1311b7f3d7c4a667be43eeb8a8249a";
    src = fetchgit {
      url = "https://github.com/mudler/vibevoice.cpp.git";
      rev = "ad856bda6b1311b7f3d7c4a667be43eeb8a8249a";
      fetchSubmodules = true;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-No0jOFb/NzWOkNzTrFK/O4D6aKJZlDnExfw+cpDWYw0=";
    };
    date = "2026-05-05";
  };
  vikunja-cli = {
    pname = "vikunja-cli";
    version = "7d96f181c2e0e07b4e75c7f540e9a8cfdf9065dc";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/vikunja-cli.git";
      rev = "7d96f181c2e0e07b4e75c7f540e9a8cfdf9065dc";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-ZxsFScCas6Uvt91eWzdwfTAsWZnVooRp/QaIp2IdHVw=";
    };
    date = "2026-02-25";
  };
  vllm = {
    pname = "vllm";
    version = "v0.22.0";
    src = fetchFromGitHub {
      owner = "vllm-project";
      repo = "vllm";
      rev = "v0.22.0";
      fetchSubmodules = false;
      sha256 = "sha256-BiaC6inRc5GJVSZMplhQYT8/RLyvFydpDAD6YZPMXTo=";
    };
  };
  web-search = {
    pname = "web-search";
    version = "d3be36dc80cc5d8f91df78ddb0d1a018d49d5e70";
    src = fetchgit {
      url = "https://github.com/p3psi-boo/web-search.git";
      rev = "d3be36dc80cc5d8f91df78ddb0d1a018d49d5e70";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-7eTnPf+pb2yFK/qrLP37GI96EqexbWDHqfNhoMYNdJg=";
    };
    date = "2026-03-21";
  };
  wikiteam3 = {
    pname = "wikiteam3";
    version = "06346a4305f8f8c5ced633c3e0dc38b3bb960e6e";
    src = fetchgit {
      url = "https://github.com/saveweb/wikiteam3.git";
      rev = "06346a4305f8f8c5ced633c3e0dc38b3bb960e6e";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-gdUZzfIn9nM60+mYacFblmyqa+pY+EbR4lPZnuy6Amk=";
    };
    date = "2025-12-24";
  };
  wsl-vpnkit = {
    pname = "wsl-vpnkit";
    version = "v0.4.1";
    src = fetchFromGitHub {
      owner = "sakai135";
      repo = "wsl-vpnkit";
      rev = "v0.4.1";
      fetchSubmodules = false;
      sha256 = "sha256-Igbr3L2W32s4uBepllSz07bkbI3qwAKMZkBrXLqGrGA=";
    };
  };
  wtfps = {
    pname = "wtfps";
    version = "44ea48854ac49396f35426246426ffcdf5616631";
    src = fetchgit {
      url = "https://codeberg.org/joelkoen/wtfps.git";
      rev = "44ea48854ac49396f35426246426ffcdf5616631";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-kWN9jOOEY1uW/1VtEYLzhm3Q2RbrjlGETk+RS2v2mTc=";
    };
    date = "2024-10-21";
  };
  xurl = {
    pname = "xurl";
    version = "24eedb2ff2b141b025efa1a1658fd7f3e88cdd01";
    src = fetchgit {
      url = "https://github.com/Xuanwo/xurl.git";
      rev = "24eedb2ff2b141b025efa1a1658fd7f3e88cdd01";
      fetchSubmodules = false;
      deepClone = false;
      leaveDotGit = false;
      sparseCheckout = [ ];
      sha256 = "sha256-M5GurO0O5LRDvFg6MtDZz40Qjdj3EKvzMiJWhVvwDwg=";
    };
    date = "2026-03-23";
  };
}
