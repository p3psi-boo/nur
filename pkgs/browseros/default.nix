# Source: https://github.com/nix-community/nur-combined/blob/main/repos/cawilliamson/pkgs/browseros/default.nix

{ appimageTools
, lib
, makeDesktopItem
, generated
, ...
}:

let
  pname = "browseros";
  version = generated.browseros.version;
  src = generated.browseros.src;

  desktopItem = makeDesktopItem {
    name = pname;
    exec = pname;
    icon = pname;
    type = "Application";
    desktopName = "BrowserOS";
    categories = [
      "Network"
      "WebBrowser"
    ];
  };

  appimageContents = appimageTools.extractType2 { inherit pname version src; };
in
appimageTools.wrapType2 {
  inherit pname src version;

  extraInstallCommands = ''
    mkdir -p $out/share/applications $out/share/icons/hicolor/256x256/apps
    cp ${desktopItem}/share/applications/* $out/share/applications
    cp ${appimageContents}/usr/share/icons/hicolor/256x256/apps/${pname}.png $out/share/icons/hicolor/256x256/apps/${pname}.png
  '';

  meta = {
    homepage = "https://www.browseros.com";
    description = "The Open source agentic browser";
    sourceProvenance = with lib.sourceTypes; [ binaryNativeCode ];
    license = lib.licenses.agpl3Only;
    platforms = [ "x86_64-linux" ];
    maintainers = with lib.maintainers; [ cawilliamson ];
    mainProgram = pname;
  };
}
